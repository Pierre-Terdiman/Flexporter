...
int ZCBClassDesc::IsPublic() { return 1; }
void* ZCBClassDesc::Create(BOOL loading) { return new ZCBImport; }
const TCHAR* ZCBClassDesc::ClassName()	{ return "ZCB importer"; }
SClass_ID ZCBClassDesc::SuperClassID() { return SCENE_IMPORT_CLASS_ID; }
Class_ID ZCBClassDesc::ClassID() { return Class_ID(1,0); }
const TCHAR* ZCBClassDesc::Category() { return "ZCB importer";  }

static ZCBClassDesc ZCBDesc;

ZCBClassDesc* IceExporter::GetZCBDesc()
{
	return &ZCBDesc;
}
...
int ZCBImport::ExtCount()
{
	return 1;
}

const TCHAR* ZCBImport::Ext(int n)
{
	if(n==0)	return "ZCB";
	else		return "";
}
const TCHAR* ZCBImport::LongDesc()
{
	return "ZCB files";
}
const TCHAR* ZCBImport::ShortDesc()
{
	return "ZCB files";
}
const TCHAR* ZCBImport::AuthorName()
{
	return "Pierre Terdiman";
}
const TCHAR* ZCBImport::CopyrightMessage()
{
	return "(C) 2002";
}
const TCHAR* ZCBImport::OtherMessage1()
{
	return "";
}
const TCHAR* ZCBImport::OtherMessage2()
{
	return "";
}
unsigned int ZCBImport::Version()
{
	return 100;
}
void ZCBImport::ShowAbout(HWND hWnd)
{
}

StdMat2* CreateNewMaterial()
{
	StdMat2* NewMtl = NewDefaultStdMat();
	if(!NewMtl)	return null;

	NewMtl->SetName(_T("Default"));
	NewMtl->SetAmbient(Color(.7f,.7f,.7f),0);
	NewMtl->SetDiffuse(Color(.7f,.7f,.7f),0);
	NewMtl->SetSpecular(Color(1.0f,1.0f,1.0f),0);
	NewMtl->SetShininess(0.5f,0);
	NewMtl->SetShinStr(.707f,0);
/*
	int shade;
	switch(smtl->shading) {
...
static uword gBunnyTriangles[BUNNY_NBFACES][3]={
	{126,134,133},
	{342,138,134},
	{133,134,138},
	{126,342,134},
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Bunny::Bunny() : mVertexNormals(null)
{
	mNbVerts	= BUNNY_NBVERTICES;
	mNbFaces	= BUNNY_NBFACES;
	mVerts		= (Point*)gBunnyVertices;
	mFaces		= (uword*)gBunnyTriangles;
}
...
bool MakeBunny(const BUNNYPRIMITIVECREATE& create, IndexedSurface* surface)
{
	...
	Bunny Rabbit;
	udword NbVerts = Rabbit.GetNbVerts();
	udword NbFaces = Rabbit.GetNbFaces();

	// New vertex cloud, new faces
	if(!surface->IndexedSurface::Init(NbFaces, NbVerts, Rabbit.GetVerts())) return false;

	// Set faces
	uword* Faces = Rabbit.GetFaces();
	for(udword i=0;i<NbFaces;i++)	surface->SetFace(Faces[i*3+0], Faces[i*3+1], Faces[i*3+2], i);
...
// Test
	if(0)
	{
		...
		Container Map, Permutation;
		udword MinRef;
		LR.ProgressiveMesh(		surface->GetNbVerts(),
								surface->GetVerts(),
								surface->GetNbFaces(),
								surface->GetFaces(),
								Map,
		...
		PermuteVertices(		surface->GetNbVerts(),
								Permutation,
								(ubyte*)surface->GetVerts(),
								sizeof(Point),
								surface->GetNbFaces(),
								(udword*)surface->GetFaces(),
								null,
								MinRef);

		Container NewMesh;
		udword NewNbVerts = surface->GetNbVerts()/8;
		udword NewNbFaces = BuildPM(
								NewNbVerts,
								Map.GetEntries(),
								surface->GetNbFaces(),
								(const udword*)surface->GetFaces(),
								null,
								NewMesh,
								MinRef);

		Point* Tmp = new Point[surface->GetNbVerts()];
		CopyMemory(Tmp, surface->GetVerts(), surface->GetNbVerts()*sizeof(Point));

		surface->Init(NewNbFaces, NewNbVerts, Tmp);
		CopyMemory((void*)surface->GetFaces(), NewMesh.GetEntries(), NewNbFaces*3*sizeof(udword));

		DELETEARRAY(Tmp);

/*		LOD_RESULT Result;
		LR.ComputeProgressiveMesh(Result);

		LR.ReorderVertexList(NbVerts, mVerts, Result.Permutation);
		LR.ReorderFaceList(NbFaces, null, (udword*)mFaces, Result.Permutation);

		CustomArray NewMesh;
		udword NewNbFaces = LR.ComputeTopology(NbFaces, null, (udword*)mFaces, 0, NbVerts/2, Result.Map, NewMesh);

		CopyMemory(mFaces, NewMesh.Collapse(), NewNbFaces*3*sizeof(udword));
		mNbFaces = NewNbFaces;
*/
	}
...
/*
	if(1)
	{
		...
		for(i=0;i<mNbVerts;i++)	Verts.Add(mVerts[i]);
		for(i=0;i<mNbFaces;i++)	Faces.Add(*(tridata*)&mFaces[i]);

		ProgressiveMesh(Verts, Faces, Map,  Permut);
		PermuteVertices(Permut, Verts, Faces);
		Container NewMesh;
		udword NewNbFaces = BuildPM(NbVerts/4, Map, Faces, NewMesh);

		CopyMemory(mFaces, NewMesh.GetEntries(), NewNbFaces*3*sizeof(udword));
		mNbFaces = NewNbFaces;
		for(i=0;i<mNbVerts;i++)	mVerts[i] = Verts[i];
#endif
	}
*/

	// Scale the model quite a bit
	surface->VertexCloud::Scale(100.0f);

	// Reset surface
	return surface->Reset();
}

bool Bunny::ComputeVertexNormals()
{
	DELETEARRAY(mVertexNormals);
	mVertexNormals = new Point[mNbVerts];
	CHECKALLOC(mVertexNormals);
	...
	nc.NbFaces	= mNbFaces;
	nc.wFaces	= mFaces;
	nc.VNormals	= mVertexNormals;

	SmoothNormals NormalComputer;
	return NormalComputer.Compute(nc);
}
#endif


BOOL ZCBFileRead(const TCHAR* filename, Mesh* msh)
{
	IceCore::MessageBox(null, "Not implemented yet!", "Sorry", MB_OK);
	...
	udword NbCVerts = 0;
	udword NbFaces = Rabbit.GetNbFaces();
	Point* Verts = Rabbit.GetVerts();

	// Vertices
	msh->setNumVerts(NbVerts);
	msh->setNumTVerts(NbTVerts);
	msh->setNumVertCol(NbCVerts);

	for(udword i=0;i<NbVerts;i++)
	{
		msh->setVert(i, Verts[i].x*10.0f, Verts[i].y*10.0f, Verts[i].z*10.0f);
	}
...
*/
	...
	// Topology
	msh->setNumFaces(NbFaces);
	msh->setNumTVFaces(0);
	msh->setNumVCFaces(0);
	for(udword i=0;i<NbFaces;i++)
	{
		msh->faces[i].setVerts(Rabbit.GetFaces()[i*3+0], Rabbit.GetFaces()[i*3+1], Rabbit.GetFaces()[i*3+2]);

		// Mark all edges as visible so that orthogonal views are correctly rendered in wireframe
		msh->faces[i].flags = EDGE_ALL;

		// Setup smoothing groups
		msh->faces[i].setSmGroup(0xffffffff);

		// Setup material ID
		msh->faces[i].setMatID(1);
	}
...
int ZCBImport::DoImport(const TCHAR* name, ImpInterface* i, Interface* gi, BOOL suppressPrompts)
{
//	IceCore::MessageBox(null, "Import", "Pipo", MB_OK);
	...
	if(ZCBFileRead(name, &object->GetMesh()))
	{
		Matrix3 tm;
		tm.IdentityMatrix();
		...
		node->SetName("ZCB object");

		node->GetINode()->SetMtl(CreateNewMaterial());
